...
class Movie(models.Model):
    ...
    )  # I dont want to store all 10.000 images locally. Due to that i prefer to store URL and send request for every image. Ideally its better to store locally.
...
class Profile(models.Model):
    ...
    avatar = models.ImageField(
        upload_to="user-avatars/", default="default-avatar/default-avatar.webp"
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "movie_app:profile_detail_view", kwargs={"profile_slug": self.slug}
        )
