...
class Command(BaseCommand):
    ...
    api_read_token = "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI1MjFhNjkzODliNGQwOTAwMmZlMDU5NWU0ZDQwMjczNyIsIm5iZiI6MTczMzMxNjkwNy4wOTcsInN1YiI6IjY3NTA1MTJiYjY2NjgzMGI2ZTQzZWE0MSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.ZylyDYYYZJOYuB00OTxA95G2H0V59xT4kLLjY9Vd5V0"  # write your api read token here.

    api_key = "521a69389b4d09002fe0595e4d402737"  # write your api key here.
    ...
    def get_video(self, movie_id):
        ...
        video_response = requests.get(
            f"https://api.themoviedb.org/3/movie/{movie_id}/videos", headers=headers
        )
        print(movie_id)
    ...
    def get_movie_genres(self, movie_genres):
        ...
        genre_names = [
            genre["name"]
            for genre in genres_response.json().get("genres")
            ...
            for movie_data in response.json().get("results"):
                try:
                    ...
                    if created:
                        ...
                        movie.save()
                        print(
                            f"{movie.title} already exist. Movie fields updated according to new data."
                        )
            ...
            for i in range(threads_per_batch):
                ...
                if page <= total_pages:
                    thread = threading.Thread(
                        target=self.get_all_videos, args=(page, page + 1)
                    )
