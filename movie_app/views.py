...
# Forms
from movie_app.forms import (ChangeUserPasswordForm, CommentForm,
                             DeleteAccountForm, ProfileUpdateForm,
                             UserLoginForm, UserSignUpForm)

from .models import Comments, Genre, Movie, Profile
...
def homepage(request):
    ...
    popular_movies = Movie.objects.order_by("-popularity").filter(is_active=True)[
        :6
    ]  # Most popular 6 Movies
    latest_movies = Movie.objects.order_by("-release_date").filter(
        is_active=True, release_date__range=("2004-01-01", today)
    )
    paginator = Paginator(latest_movies, 20)
    page_number = request.GET.get("page")
    last_movies = paginator.get_page(
        page_number,
    )
...
def category_view(request, category_slug):
    ...
    context = dict(
        category=category, category_movies=page_obj, page_title=category.title
    )
...
def login_view(request):
    if request.user.is_authenticated:
        messages.info(
            request, f"Welcome {request.user.get_full_name()} You Already Logged in."
        )
        return redirect("/")
    ...
    if form.is_valid():
        ...
        if user is not None:
            login(request, user)
            messages.success(
                request, f"{request.user.username} You are succesfully logged in."
            )
            return redirect("/")
        messages.warning(
            request, "Wrong password or username. Please check the info and try again."
        )
...
def change_password_view(request):
    ...
    if form.is_valid():
        ...
        if user.check_password(
            password
        ):  # this can be done in forms.py form validation but i prefer to make it on view level.
            user.set_password(new_password)  # hashing string password data.
            user.save()
            messages.success(
                request, f"{user.username} your password changed succesfully."
            )
...
def delete_account_view(request):
    ...
    if form.is_valid():
        ...
        messages.warning(
            request, "Wrong confirmation number. Please check and try again."
        )
    ...
    context = dict(
        form=form,
        confirmation_int=confirmation_int,
        page_title=f"{request.user.username} Account Delete",
    )
